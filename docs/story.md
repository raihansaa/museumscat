...
We did not read a single label until the measurement apparatus existed. That was probably the
best decision of the campaign, because almost every later result is a small difference between
two numbers, and small differences are only trustworthy if the numbers are.
...
The candidate archive mattered more than it sounds. Because every read was cached, most of the
later experiments cost nothing: re-scoring a cached reader under a new gate is free, and we ran
dozens of those. The savings came from the archive rather than from being careful about spend.
...
- **`src/crop_tag.py`**: the one we used. It detects the pale label rectangles by brightness
  and crops tightly to them.

Reading was local at first, through **Ollama**, using **`src/probe_vlm.py`**, a JSON-emitting
prompt over the crops at temperature 0. The first working reader was **qwen2.5-VL-7B**.
...
The first cloud reader was **qwen3-VL-32B**, at a measured $0.00026 per image, or $0.86 for the
...
Variant E is the instructive one. The 32B did transcribe dates more accurately
...
it structurally cannot touch the 2–1–1 stratum, which the measurement said was the largest prize.
Replacing the text directly does reach it.
...
A few operational details that turned out to matter:
...
- **`src/gemini_filter.py`**: Gemini's one systematic defect is inventing macrons
  ...
  named-character table rather than an NFD strip, because a blanket strip destroys Danish `Å`
  (Århus → Arhus).
- **`harvest_reads.sh`**: the direct Gemini API free tier allows 20 requests per day per model,
  so rotating a validated pool of six Gemini variants yields around 120 free validated reads a
  day. Resumable, and it retires a model on a per-day 429.
...
probabilities from Ollama logprobs. Later, a full visual-grounding line: the score really is
...
query **hurt** by +0.0018. It appears to pattern-match against the exemplars instead of reading.
Stopped at the labelled gate for $0.55; no submission slot spent.

**Image enhancement and alternative OCR.** Sharpening, contrast enhancement, RapidOCR. All null.
The error taxonomy explains why: 49% of locality loss is multi-card segment selection rather than
...
happened to include a control arm that changed nothing. We would always include one now.

In hindsight this is also the largest lever we left on the table. We recorded the variance as
noise to defend against. It is just as much a signal to exploit: our two-draw self-disagreement
...
expected scores. The runner-up by score fails in the same world the leader fails. We picked the
